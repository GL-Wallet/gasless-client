...
export const fee = router('fees', {
  getFeesDepositAddress: router.query<
    FeesDepositAddressDto,
    { network: string, asset: string }
  >({
    fetcher: ({ network, asset }) =>
      api
        .get(`fees/deposit-address`, { params: { network, asset } })
  ...
  getFeesRange: router.query<FeesRangeDto, { network: string, asset: string }>({
    fetcher: ({ network, asset }) =>
      api
        .get(`fees/range`, { params: { network, asset } })
